  ...
  return (
    <main className="resume-page resume-page-simple">
      ...
      <section className="resume-simple-grid">
        <section><h2>Theater</h2><div data-editable-field="resume.theater" data-editable-kind="richText"><Credits items={rows(content.resume.theater)} /></div><h2>Staged readings</h2><div className="resume-simple-list" data-editable-field="resume.stagedReadings" data-editable-kind="richText">{rows(content.resume.stagedReadings).map(([title, detail]) => <div key={title}><strong dangerouslySetInnerHTML={{ __html: richTextHtml(title) }} /><small>{detail}</small></div>)}</div></section>
        <section><h2>Film + screen</h2><div data-editable-field="resume.film" data-editable-kind="richText"><Credits items={rows(content.resume.film)} /></div><h2>Training</h2><div className="resume-simple-list" data-editable-field="resume.training" data-editable-kind="richText">{rows(content.resume.training).map(([name, detail]) => <div key={name}><strong dangerouslySetInnerHTML={{ __html: richTextHtml(name) }} /><small>{detail}</small></div>)}</div><h2>Special skills</h2><EditableRichText field="resume.skills" value={content.resume.skills} /></section>
      </section>
    </main>
  );
}
