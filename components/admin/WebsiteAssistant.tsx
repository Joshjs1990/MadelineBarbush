...
export function WebsiteAssistant() {
  ...
  return <section className="website-assistant" aria-labelledby="website-assistant-title">
    <div className="website-assistant__intro"><p className="eyebrow">Website Assistant</p><h2 id="website-assistant-title">Website Assistant</h2><p>Ask about the current site, request a copy change, or get pointed to the right editor. Changes always show a preview before anything is applied.</p><p className="website-assistant__note">Colours and fonts are managed in the sections below.</p></div>
    <div className="website-assistant__workspace">
      ...
      <form className="website-assistant__form" onSubmit={ask}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Ask a question or describe a change…" maxLength={2000} rows={3} disabled={busy} /><button type="submit" disabled={busy || !draft.trim()}>{busy ? "Working…" : "Send"}</button></form>
