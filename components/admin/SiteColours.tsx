...
const palette = [
  ["theme.primary", "Primary accent", "Highlights, buttons and active states"], ["theme.secondary", "Secondary accent", "A supporting colour for editorial moments"], ["theme.background", "Background", "The sitewide page surface"], ["theme.foreground", "Foreground / text", "Main text and dark surfaces"], ["theme.highlight", "Highlight", "Links and keyboard focus states"],
] as const;
...
export function SiteColours({ content }: { content: EditableContent }) {
  ...
  const save = async (nextValues: Record<string, string>, ids: EditableFieldId[] = palette.map(([id]) => id as EditableFieldId)) => {
    ...
    setValues(nextValues); setBaseline(nextValues); setStatus(body.data?.message ?? "Colours saved."); window.dispatchEvent(new Event("assistant-history-updated")); window.location.reload();
  };
  const resetOne = (id: EditableFieldId) => void save({ ...values, [id]: fieldValue(DEFAULT_EDITABLE_CONTENT, id) }, [id]);
