...
export function EditableImage({ field, src, alt, focalX = 50, focalY = 50, fit = "cover", className, fill, sizes, priority = false }: { field: string; src: string; alt: string; focalX?: number; focalY?: number; fit?: "cover" | "contain"; className?: string; fill?: boolean; sizes?: string; priority?: boolean }) {
  ...
  return <img className={className} data-editable-field={field} data-editable-kind="image" data-editable-fit={fit} src={src} alt={alt} sizes={sizes} width={fill ? undefined : 1200} height={fill ? undefined : 1200} loading={priority ? "eager" : "lazy"} fetchPriority={priority ? "high" : undefined} decoding="async" style={style} />;
}
