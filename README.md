...
## Bookings, Clients and Email

`/book` is a public booking area with its own client accounts, separate from the
admin logins. A client registers, requests a date, and hears back by email. The
admin side is a small CRM at `/admin/bookings` and `/admin/clients`.

### The flow

1. A client registers at `/book`. They get a welcome email; every active
   administrator gets a "new account" email.
2. They request a date. The request is saved as `requested` — the status is set
   server-side, so a client cannot request its way to confirmed. They get a
   receipt, the admins get the request with a reply-to of the client.
3. An admin confirms or declines from `/admin/bookings`. Confirming emails the
   client with the `.ics` invite attached; declining and cancelling each send
   their own note. Re-saving details does not re-send anything.
4. The day before, a reminder goes to the client and the admins.

Add-to-calendar links (Google, and `.ics` for Apple/Outlook) appear anywhere a
confirmed booking is shown, and in the emails.

### Configuration

All optional — without them bookings still work, and email degrades to a log
line rather than an error.

| Variable | Purpose |
| --- | --- |
| `RESEND_API_KEY` | Enables sending. Without it, messages are composed and logged, and `/admin/bookings` says so. |
| `BOOKING_FROM_EMAIL` | From address. Must be on a domain verified in Resend. Defaults to `bookings@thecoolmoon.com`. |
| `CRON_SECRET` | Required by the reminder endpoint. Without it that endpoint refuses to run rather than defaulting to open. |

In production set these as Cloudflare secrets — they arrive on the worker `env`.
For local development, export them before `npm run dev` and `vite.config.ts`
forwards them into the worker. That forwarding is deliberately limited to
`command === "serve"`, so a build never writes a secret into the deployed
worker's plain-text vars.

### Reminders

Reminders are sent by an endpoint, not a built-in schedule, so any scheduler can
drive them:

```bash
curl -X POST https://<site>/api/cron/reminders \
  -H "authorization: Bearer $CRON_SECRET"
```

It looks for confirmed bookings starting 24–48 hours out that have not been
reminded yet. Each booking is marked before its email goes out, so calling this
hourly, daily, or twice by accident never sends anyone two reminders. The
response reports what it sent and what it skipped.

Point cron-job.org, a GitHub Action, or a Cloudflare cron trigger at it — once a
day is enough, given the window.
