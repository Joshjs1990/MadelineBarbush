...
export function sanitizeHighlightsHtml(input: string) {
  ...
  // Older editor output could wrap several block elements in one heading.
  // Remove only that invalid wrapper; valid standalone headings stay intact.
  const hasInvalidHeadingWrapper = /<h[23]>(?=\s*<(?:p|h[23]|ul|ol)\b)/i.test(sanitized);
  const normalized = sanitized.replace(/<h[23]>(?=\s*<(?:p|h[23]|ul|ol)\b)/gi, "");
  return (hasInvalidHeadingWrapper ? normalized.replace(/<\/h[23]>\s*$/i, "") : normalized)
    .trim()
    .slice(0, 20000);
}
export async function getRecentHighlights() { try { const stored = await readSetting(HIGHLIGHTS_KEY); return stored ? sanitizeHighlightsHtml(stored) : RECENT_HIGHLIGHTS_DEFAULT; } catch { return RECENT_HIGHLIGHTS_DEFAULT; } }
export async function saveRecentHighlights(input: string) { const html = sanitizeHighlightsHtml(input); if (!html) throw new Error("Add some highlights before saving."); await writeSetting(HIGHLIGHTS_KEY, html); return html; }
